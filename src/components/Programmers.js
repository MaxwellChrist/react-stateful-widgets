...
export default function Programmers() {
  ...
  // const [programmers, setProgrammers] = useState(['Ada Lovelace', 'Grace Hopper', 'Evelyn Boyd Granville', 'Mary Kenneth Keller', 'Frances Allen', 'Carol Shaw']);
      ...
      <div className='programmers'>
        {
          ...
          programmers.map(dev =>
            <div className='programmer' key={dev.id}>
              {dev.name} <button onClick={() => setIds(dev.id) /* in here set the featured id to be dev.id */ }>Feature</button>
            </div>
          )
        }
      </div>
